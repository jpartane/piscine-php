<?php
    ...
    foreach ($resLogQuery as $elem) {
        ...
        echo "</tr>";
        $total = $total + (float)$elem['price'];
        $i++;
    }
    echo "<tr><td style='color: rgb(128,0,32); height: 70px; font-weight: bold'>Total: </td><td style='color: rgb(128,0,32); height: 70px;'>$total €</td><td><form method='post'><input style='padding: 10px; margin-top: 10px; width: 150px' type='submit' name='order' value='Order'></form></td></tr>";
    echo "</table>";
    echo "</div>";
    echo "</div>";
    ...
    if (isset($_POST['order'])) {
        if ($_POST['order'] == 'Order') {
            if (!$_SESSION['loggued_on_user']) {
                ...
                echo "<div><h2 style='color: green'>Your order has been dispatched</h2></div>";
            }
        }
    }