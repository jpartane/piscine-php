<?php
    ...
    foreach ($resQuery as $elem) {
        ...
        foreach ($elem as $value) {
            if ($value == $elem['img']) {
                echo "<img src='$value' alt='wine' width='100' height='100'>";
            } else {
    ...
    if (isset($_POST['modify_item'])) {
        if ($_POST['modify_item'] == "Edit") {
            ...
            $price = mysqli_real_escape_string($link, $_POST['2']);
            $category = mysqli_real_escape_string($link, $_POST['3']);
            $subcategory = mysqli_real_escape_string($link, $_POST['4']);
            $description = mysqli_real_escape_string($link, $_POST['5']);

            $query = mysqli_query($link, "UPDATE `items` SET `name` = '$name', `price` = '$price', `category` = '$category', `subcategory` = '$subcategory', `description` = '$description' WHERE id = '$id'");
            refresh();
        }
    }